$(document).ready(function(){
  ...
  var table_companies = $('#table_companies').dataTable({
    ...
    "columns": [
      { "data": "id" },
      { "data": "heading",   "sClass": "heading" },
      { "data": "claim_no" },
      { "data": "description",        "sClass": "string" },
  ...
  $(document).on('click', '#add_company', function(e){
    e.preventDefault();
    $('.lightbox_content h2').text('Add notes');
    $('#form_company button').text('Add notes');
    $('#form_company').attr('class', 'form add');
    $('#form_company').attr('data-id', '');
    $('#form_company .field_container label.error').hide();
    $('#form_company .field_container').removeClass('valid').removeClass('error');
    $('#form_company #heading').val('');
    $('#form_company #claim_no').val('');
  ...
  $(document).on('click', '.function_edit a', function(e){
    ...
    var request = $.ajax({
      ...
      if (output.result == 'success'){
        ...
        $('#form_company .field_container label.error').hide();
        $('#form_company .field_container').removeClass('valid').removeClass('error');
        $('#form_company #heading').val(output.data[0].heading);
        $('#form_company #claim_no').val(output.data[0].claim_no);
      ...
      request.done(function(output){
        if (output.result == 'success'){
          ...
          table_companies.api().ajax.reload(function(){
            hide_loading_message();
            var company_name = $('#heading').val();
            show_message("Company  edited successfully.", 'success');
          }, true);
        } else {
          hide_loading_message();
